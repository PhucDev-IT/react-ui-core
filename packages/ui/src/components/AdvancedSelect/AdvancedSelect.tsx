...
function Chevron({right=false}:{right?:boolean}){
 return <svg className={cx('ui-adv-select__chevron',right&&'is-right')} viewBox="0 0 20 20" aria-hidden><path d={right?'m7.5 5 5 5-5 5':'m5 7.5 5 5 5-5'}/></svg>;
}
...
export function Cascader({label,hint,options,value=[],onChange,placeholder='Chọn danh mục'}:{label?:React.ReactNode;hint?:React.ReactNode;options:Option[];value?:string[];onChange?:(path:string[])=>void;placeholder?:string}){
 ...
 return <Field label={label} hint={hint}><div className="ui-adv-select"><button type="button" className="ui-adv-select__trigger" onClick={()=>setOpen(v=>!v)}><span>{pathLabels.length?pathLabels.join(' / '):placeholder}</span><Chevron/></button>{open&&<div className="ui-cascader">{columns.map((col,depth)=><div key={depth}>{col.map(item=><button type="button" key={item.value} className={value[depth]===item.value?'is-selected':''} onClick={()=>pick(depth,item)}>{item.label}{item.children?.length?<Chevron right/>:null}</button>)}</div>)}</div>}</div></Field>
}
